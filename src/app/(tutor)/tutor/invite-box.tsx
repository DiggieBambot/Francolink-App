  ...
  return (
    <div className="bg-gradient-to-br from-blue-600 to-blue-800 rounded-xl p-6 text-white shadow-lg h-full">
      ...
      <div className="bg-white/10 backdrop-blur rounded-lg p-3 flex items-center gap-3 mb-4 border border-white/20">
        <code className="font-mono text-xs sm:text-sm truncate flex-1 text-blue-50 h-5">
          {displayLink || 'Loading...'}
        </code>
        <button 
          onClick={handleCopy}
          disabled={!fullLink}
          className="flex items-center gap-1.5 px-3 py-1.5 bg-white text-blue-700 rounded-md text-xs font-bold hover:bg-blue-50 transition-all active:scale-95 shadow-sm disabled:opacity-50 disabled:cursor-not-allowed"
        >
          {copied ? (
            <>
