...
export function SpeakButton({ text, size = "md", variant = "icon", className = "", voice }: SpeakButtonProps) {
  ...
  const Icon = tts.isLoading ? Loader2 : tts.isSpeaking ? Square : Volume2;
  const iconClass = tts.isLoading ? "animate-spin" : tts.isSpeaking ? "text-blue-600" : "";

  return (
    <button
      type="button"
      onClick={handleClick}
      aria-label={tts.isSpeaking ? `Stop: ${text}` : `Play: ${text}`}
      title={tts.isSpeaking ? "Stop" : "Play"}
      className={
        variant === "inline"
