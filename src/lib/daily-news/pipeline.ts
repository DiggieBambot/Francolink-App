        ...
        messages: [
          {
            ...
            content:
              ...
              `- vocabulary: EXACTLY 6 items {word, ipa, part_of_speech, definition, example} — word/definition/example in ${config.language === "fr" ? "French" : "English"}; part_of_speech stays in English (e.g. "noun", "verb"); choose useful words that appear in your article; the example must use the word in a natural sentence.${
                config.language === "fr"
                  ? ` FRENCH NOUNS MUST CARRY THEIR ARTICLE in "word", so the learner sees the gender: "le lancement", "la croissance", "l'écran", "les données" — never a bare "lancement". Use l' before a vowel or mute h ("l'hôtel") but le/la before an aspirate h ("le héros"), and les for nouns that are normally plural. Pick ONE article and write it out — never "le/la lancement". Leave the article off verbs, adjectives and phrases.`
                  : ""
              }\n` +
